                ...
                <div class="nav-align-top mb-4">
                    <ul class="nav nav-tabs" role="tablist">
                        <li class="nav-item">
                            <button type="button" class="nav-link active" role="tab" data-bs-toggle="tab"
                                data-bs-target="#navs-top-dapur-saya" aria-controls="navs-top-dapur-saya"
                                aria-selected="true">
                        ...
                        <li class="nav-item">
                            <button type="button" class="nav-link" role="tab" data-bs-toggle="tab"
                                data-bs-target="#navs-top-tambah-dapur" aria-controls="navs-top-tambah-dapur"
                                aria-selected="false">
                    ...
                    <div class="tab-content">
                                        ...
                                        @forelse($mitraDapurList as $index => $md)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @if ($md->catatan)
                                                        <br><small class="text-danger mt-1 d-block"><i
                                                                class="bx bx-info-circle"></i> {{ $md->catatan }}</small>
                                                    @endif
                                                </td>
                                                <td>{{ $md->created_at->format('d M Y, H:i') }}</td>
                                                <td>
                                                    @if (!$md->isApproved())
                                                        <form action="{{ route('mitra.dapur.destroy', $md) }}"
                                                            method="POST"
                                                            onsubmit="return confirm('Yakin ingin membatalkan/menghapus pengajuan ini?');">
                                                            @csrf
                                                            @method('DELETE')
                                                    ...
                                                    @else
                                                        <a href="{{ route('mitra.dapur.show', $md) }}"
                                            ...
                                            <tr>
                                                <td colspan="5" class="text-center py-4 bg-light">
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-primary mt-2"
                                                        onclick="$('.nav-tabs button[data-bs-target=\'#navs-top-tambah-dapur\']').tab('show')">
                        ...
                        <div class="tab-pane fade" id="navs-top-tambah-dapur" role="tabpanel">
                            ...
                            <p class="text-muted mb-4">Cari nama Dapur SPPG yang ingin Anda ikuti, kemudian klik "Ajukan
                                Dapur" untuk mengirimkan permohonan ke Kepala Dapur terkait.</p>

                            <form action="{{ route('mitra.dapur.store') }}" method="POST" id="bulk_dapur_form">
                                @csrf
                                <div class="row g-3">
                                    <div class="col-md-12">
                                        <label for="dapur_search" class="form-label fw-semibold">Cari & Pilih Dapur SPPG
                                            (Bisa lebih dari satu)</label>
                                        <div class="position-relative">
                                            <div class="input-group input-group-merge shadow-sm border-primary">
                                                <span class="input-group-text bg-white border-end-0"><i
                                                        class="bx bx-search fs-4 text-primary"></i></span>
                                                <input type="text"
                                                    class="form-control form-control-lg border-start-0 ps-0"
                                                    id="dapur_search"
                                                    placeholder="Ketik nama dapur (contoh: Dapur Jakarta)..."
                                                    autocomplete="off">
                                                <button class="btn btn-outline-secondary border-start-0" type="button"
                                                    id="clear_dapur"><i class="bx bx-x"></i></button>
                                            </div>
                                            <div id="dapur_dropdown"
                                                class="list-group position-absolute w-100 shadow-lg mt-1 animate__animated animate__fadeInUp"
                                                style="z-index: 1100; display: none; max-height: 350px; overflow-y: auto; border-radius: 0.75rem; border: 1px solid #eee;">
                                            </div>
                                        </div>
                                        ...
                                        @error('id_dapur')
                                            <div class="text-danger small mt-2"><i class="bx bx-error-circle me-1"></i>
                                                {{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-12 text-end mt-4">
                                        <button type="submit" class="btn btn-primary btn-lg px-5 shadow-sm"
                                            id="btn_submit" disabled>
                                            <i class="bx bx-paper-plane me-1"></i> Ajukan <span id="selected_count"
                                                class="badge bg-white text-primary ms-1" style="display: none;">0</span>
                                            Dapur
                                        </button>
                                    </div>
                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#dapur_search').on('input', function() {
                ...
                $('#dapur_dropdown').html(
                    '<div class="list-group-item text-center py-3"><div class="spinner-border spinner-border-sm text-primary" role="status"></div><span class="ms-2 text-muted">Mencari...</span></div>'
                    ).show();
                ...
                dapurSearchTimeout = setTimeout(function() {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length === 0) {
                                dropdown.append(
                                    '<div class="list-group-item disabled text-center py-4 text-muted"><i class="bx bx-search-alt fs-2 d-block mb-2 opacity-50"></i>Dapur tidak ditemukan. Pastikan nama yang Anda ketik benar.</div>'
                                    );
                            } else {
                                data.forEach(function(d) {
                                    const isAlreadySelected = selectedDapurs
                                        .has(d.id_dapur.toString());
                                    dropdown.append(`
            ...
            `);
                ...
                $('#hidden_inputs_container').append(
                    `<input type="hidden" name="id_dapur[]" value="${id}" id="input_${id}">`);
