            ...
            <form action="{{ route('akuntan.transaksi.store') }}" method="POST" id="trx-form">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="date" name="date" class="form-control" value="{{ old('date', date('Y-m-d')) }}" required id="trx-date">
                    </div>
                    <div class="col-12 col-md-6">
                        <label class="form-label fw-semibold">No. Bukti</label>
                        <input type="text" name="no_bukti" class="form-control" value="{{ old('no_bukti') }}" placeholder="e.g. BKU-001/2024">
                    </div>
                    <div class="col-12 col-md-6">
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="description" class="form-control" value="{{ old('description') }}" required placeholder="Deskripsi transaksi">
                    </div>
                    <div class="col-12 col-md-6">
                        ...
                        <select name="cash_account_id" class="form-select">
                            ...
                            @foreach($cashAccounts as $ca)
                                <option value="{{ $ca->id }}" {{ old('cash_account_id') == $ca->id ? 'selected' : '' }}>{{ $ca->name }} ({{ $ca->type_label }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-12">
                    ...
                    <div class="col-12 col-md-6" id="debit-container">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="debit" id="debit-field" class="form-control" value="{{ old('debit', 0) }}" min="0" step="1">
                        </div>
                    </div>
                    <div class="col-12 col-md-6" id="credit-container">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="number" name="credit" id="credit-field" class="form-control" value="{{ old('credit', 0) }}" min="0" step="1">
                        </div>
                    </div>
                    <div class="col-12 d-flex gap-2">
                        <button type="submit" class="btn btn-primary"><i class="bx bx-save me-1"></i>Simpan Transaksi</button>
...
async function updateBalancePreview() {
    ...
    try {
        const response = await fetch(`{{ route('akuntan.transaksi.getBalance') }}?period_id=${periodId}&cash_account_id=${cashAccountId}`);
