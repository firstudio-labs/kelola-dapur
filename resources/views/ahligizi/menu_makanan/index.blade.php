        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="mb-0 text-muted">Kelola semua menu makanan dalam sistem</p>
                </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('ahli-gizi.menu-makanan.index') }}" class="row g-3">
                <div class="col-md-4">
                    <label for="search-input" class="form-label">Cari Menu</label>
                    <div class="input-group">
                        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
                               class="form-control" placeholder="Cari nama menu atau deskripsi...">
                ...
                <div class="col-12 d-flex justify-content-end gap-2 mt-3">
                    @if(request()->has('search') || request()->has('status'))
                        <a href="{{ route('ahli-gizi.menu-makanan.index') }}" class="btn btn-outline-secondary">Reset Filter</a>
                    @endif
                    ...
                    <div class="col-md-4 text-center">
                        <div class="d-flex align-items-center justify-content-center">
                            ...
                            <div>
                                ...
                                <h6 class="mb-0">{{ $menus->where('is_active', true)->count() }}</h6>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 text-center">
                        <div class="d-flex align-items-center justify-content-center">
                            ...
                            <div>
                                ...
                                <h6 class="mb-0">{{ $menus->where('is_active', false)->count() }}</h6>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody id="menu-table-body">
                            @foreach($menus as $menu)
                                <tr data-search="{{ strtolower($menu->nama_menu . ' ' . $menu->deskripsi) }}" data-status="{{ $menu->is_active ? '1' : '0' }}">
                                    <td>
                                        <img src="{{ $menu->gambar_url }}" alt="{{ $menu->nama_menu }}" class="rounded" style="width: 60px; height: 40px; object-fit: cover;">
                                    </td>
                                    <td>{{ $menu->nama_menu }}</td>
                                    <td>{{ Str::limit($menu->deskripsi, 50) }}</td>
                                    <td>{{ $menu->bahanMenu->count() }} bahan</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="{{ route('ahli-gizi.menu-makanan.show',  $menu) }}" class="btn btn-sm btn-outline-primary action-btn" data-bs-toggle="tooltip" title="Lihat Detail">
                                                <i class="bx bx-show"></i>
                                            </a>
                                            <a href="{{ route('ahli-gizi.menu-makanan.edit', $menu) }}" class="btn btn-sm btn-outline-info action-btn" data-bs-toggle="tooltip" title="Edit Menu">
                                                <i class="bx bx-edit"></i>
                                            </a>
                                            <form action="{{ route('ahli-gizi.menu-makanan.toggle-status',  $menu) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="btn btn-sm btn-outline-{{ $menu->is_active ? 'danger' : 'success' }} action-btn" data-bs-toggle="tooltip" title="{{ $menu->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                                    <i class="bx {{ $menu->is_active ? 'bx-block' : 'bx-check-circle' }}"></i>
                                                </button>
                                            </form>
                                        </div>
                                    </td>
            ...
            @else
                ...
                <div class="text-center py-6">
                    @if(request()->hasAny(['search', 'status']))
                        ...
                        <p class="text-muted mb-3">Tidak ada menu yang sesuai dengan filter.</p>
                    ...
                    @else
                        ...
                        <p class="text-muted mb-3">Mulai dengan membuat menu pertama.</p>
                        <a href="{{ route('ahli-gizi.menu-makanan.create') }}" class="btn btn-primary">
                            <i class="bx bx-plus me-1"></i>Tambah Menu Pertama
                        </a>
                    @endif
                </div>
            @endif
