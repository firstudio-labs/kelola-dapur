            ...
            <div class="card-body">
                @if ($penerima->status_approval !== 'approved')
                    <div class="alert alert-warning">
                        ...
                        Pengajuan Anda belum disetujui. Perubahan jumlah porsi hanya dapat dilakukan setelah disetujui oleh
                        Dapur SPPG.
                    </div>
                @endif
                ...
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        ...
                        <form action="{{ route('penerima-mbg.porsi.update') }}" method="POST">
                            ...
                            @method('PUT')
                            <div class="mb-3">
                                <label for="jumlah_porsi" class="form-label fw-semibold">Jumlah Porsi Baru <span
                                ...
                                <input type="number" name="jumlah_porsi" id="jumlah_porsi"
                                    ...
                                    value="{{ old('jumlah_porsi', $penerima->jumlah_porsi) }}" min="1" max="9999999"
                                    required {{ $penerima->status_approval !== 'approved' ? 'disabled' : '' }}>
                                @error('jumlah_porsi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text text-center">Jumlah porsi yang Anda butuhkan per hari (1–1.000.000
                                    porsi)</div>
                            </div>
                            <div class="d-grid gap-2">
                                ...
                                @endif
                                <a href="{{ route('penerima-mbg.dashboard') }}" class="btn btn-label-secondary">Kembali ke
                                    Dashboard</a>
                            </div>
                        </form>
                    </div>
