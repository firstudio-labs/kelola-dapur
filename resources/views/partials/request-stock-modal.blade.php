                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" id="modalBahanName" class="form-control"
                                value="{{ isset($stockItem) ? $stockItem->templateItem->nama_bahan : '' }}" readonly />
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Stok Saat Ini</label>
                            <div class="input-group">
                                <input type="text" id="modalCurrentStock" class="form-control"
                                    value="{{ isset($stockItem) ? (float) $stockItem->jumlah : '' }}" readonly />
                                <span class="input-group-text" id="modalCurrentSatuan">
                                    {{ isset($stockItem) ? $stockItem->templateItem->satuan : '' }}
                                </span>
                            </div>
                            <div id="modalCurrentStockKonversi" class="form-text text-primary mt-1"
                                style="{{ isset($stockItem) && $stockItem->konversi_nilai > 0 ? 'display: block;' : 'display: none;' }}">
                                @if (isset($stockItem) && $stockItem->konversi_nilai > 0)
                                    Setara dengan:
                                    {{ formatIndonesianNumber($stockItem->jumlah / $stockItem->konversi_nilai) }}
                    ...
                    <div class="mb-3 d-none bg-light p-2 rounded" id="konversiToggleContainer">
                        <label class="form-label d-block mb-1 fw-medium" style="font-size: 0.85rem;">Format Input
                            Satuan:</label>
                        <div class="d-flex gap-3">
                            <div class="form-check form-check-inline mb-0">
                                <input class="form-check-input unit-toggle-radio" type="radio" name="input_mode"
                                    id="unit_asli" value="asli" checked>
                                <label class="form-check-label" for="unit_asli" id="label_unit_asli"
                                    style="font-size: 0.85rem;">Satuan Asli (-)</label>
                            </div>
                            <div class="form-check form-check-inline mb-0">
                                <input class="form-check-input unit-toggle-radio" type="radio" name="input_mode"
                                    id="unit_konversi" value="konversi">
                                <label class="form-check-label text-primary" for="unit_konversi"
                                    id="label_unit_konversi" style="font-size: 0.85rem;">Konversi (-)</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" name="jumlah" id="jumlah"
                                class="form-control @error('jumlah') is-invalid @enderror" required placeholder="0"
                                value="{{ old('jumlah') }}" />
                    ...
                    <div class="mb-3">
                        <label class="form-label d-flex justify-content-between align-items-center">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-primary" id="addSupplierRowBtn"
                                onclick="window.createSupplierRow()">
                            ...
                            <table class="table table-sm table-borderless mb-0" id="supplierTable">
                                ...
                                <tfoot>
                                    <tr class="table-light border-top">
                                        ...
                                        <td colspan="2">
                                            <span id="totalSupplierUi">0</span> <span
                                                id="modalSatuanBottom">{{ isset($stockItem) ? $stockItem->templateItem->satuan : '' }}</span>
                                            <small class="text-danger d-block d-none" id="supplierErrorMsg">Total
                                                melebihi Total Penambahan Stok!</small>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                        <small class="form-text text-muted">Jika tidak ada rincian supplier, sistem akan mencatat stok
                            ke asal Gudang Umum.</small>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" name="suhu_bahan_makanan" id="suhu_bahan_makanan"
                                    class="form-control @error('suhu_bahan_makanan') is-invalid @enderror"
                                    step="0.01" min="-50" max="100" placeholder="0.00"
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="warna_bahan_makanan" id="warna_bahan_makanan"
                                class="form-control @error('warna_bahan_makanan') is-invalid @enderror" maxlength="50"
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="file" name="foto_bahan" id="foto_bahan"
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                            rows="3" maxlength="500" placeholder="Alasan penambahan stok (opsional)...">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="bg-light rounded p-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                ...
                                <div class="fw-medium text-success" id="previewStock">
                                    -
                                </div>
                            </div>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const bindQuantityEvents = (input) => {
            ...
            input.addEventListener('input', function() {
                ...
                if (integerPart) {
                    ...
                    formattedInt = new Intl.NumberFormat('id-ID').format(parseInt(integerPart) ||
                    0);
                } else {
                    formattedInt = "0";
                }
                ...
                this.setSelectionRange(cursorPosition + (newLength - originalLength),
                    cursorPosition + (newLength - originalLength));
            });

            input.addEventListener('focus', function() {
                if (this.value === "0" || this.value === "0,0" || this.value === "0,00" || this
                    .value === "0,000" || this.value === "0,0000") {
                    this.value = "";
                }
            });
        ...
        @if (isset($stockItem))
            ...
            if (window.activeKonversiNilai > 0 && window.activeKonversiSatuan !== '') {
                ...
                if (labelUnitKonversi) labelUnitKonversi.textContent =
                    `Konversi (${window.activeKonversiSatuan})`;
            }
        @else
            let currentStock = 0;
            let satuan = '';
        ...
        if (jumlahInput && previewStock) {
            ...
            window.updatePreview = function() {
                ...
                let previewText = formatIndonesianNumberJS(newStock) + ' ' + (satuan || '');
                if (window.activeKonversiNilai > 0 && window.activeKonversiSatuan !== '') {
                    const convertedNewStock = newStock / window.activeKonversiNilai;
                    previewText +=
                        ` (${formatIndonesianNumberJS(convertedNewStock)} ${window.activeKonversiSatuan})`;
                }
                previewStock.textContent = previewText;
        ...
        @isset($suppliers)
            @foreach ($suppliers as $supplier)
                supplierOptionsHtml += '<option value="{{ $supplier->id_supplier }}">' +
                    {!! json_encode(htmlspecialchars($supplier->nama_supplier)) !!} + '</option>';
            @endforeach
        @endisset
        ...
        window.updateRequestStockModal = function(stockId, bahanName, currentStockValue, satuanValue,
            defaultJumlah = '', konversiNilaiVal = 0, konversiSatuanVal = '') {
            ...
            let previewCurrentText = parseFloat(currentStockValue).toString() + ' ' + satuanValue;
            const kVal = parseFloat(konversiNilaiVal) || 0;
            const currentStockKonversiDiv = document.getElementById('modalCurrentStockKonversi');
            if (currentStockKonversiDiv) {
                if (kVal > 0 && konversiSatuanVal) {
                    const convertedVal = parseFloat(currentStockValue) / kVal;
                    currentStockKonversiDiv.textContent =
                        `Setara dengan: ${formatIndonesianNumberJS(convertedVal)} ${konversiSatuanVal}`;
                    currentStockKonversiDiv.style.setProperty('display', 'block', 'important');
                    previewCurrentText +=
                        ` (${formatIndonesianNumberJS(convertedVal)} ${konversiSatuanVal})`;
                } else {
                    currentStockKonversiDiv.style.setProperty('display', 'none', 'important');
                }
            }
            if (previewCurrentStock) {
                previewCurrentStock.textContent = previewCurrentText;
            }
            if (requestStockForm) {
        ...
        unitRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                const targetedUnitLabel = newMode === 'asli' ? satuan : window
                    .activeKonversiSatuan;
                ...
                supplierInputs.forEach(input => {
                    if (input.value !== '') {
                        let scaledVal = parseFormattedDecimalJS(input.value) *
                            multiplier;
