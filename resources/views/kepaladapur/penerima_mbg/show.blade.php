    ...
    <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h4 class="mb-1">Detail Penerima MBG</h4>
                        <p class="mb-0 text-muted">Informasi detail mengenai pengajuan penerima MBG</p>
                    </div>
                    <div>
                        <a href="{{ route('kepala-dapur.penerima-mbg.index', $dapur) }}" class="btn btn-outline-secondary">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-4">
                    ...
                    @php
                        $col = match ($penerima_mbg->status_approval) {
                            'approved' => 'success',
                            'rejected' => 'danger',
                            default => 'warning',
                        };
                    @endphp
                    <span
                        class="badge bg-label-{{ $col }} fs-6">{{ ucfirst($penerima_mbg->status_approval) }}</span>
                </div>

                <div class="row g-4">
                    <div class="col-md-3 text-center">
                        @if ($penerima_mbg->foto_lokasi)
                            <img src="{{ Storage::url($penerima_mbg->foto_lokasi) }}" class="img-fluid rounded"
                                alt="Foto Lokasi" style="max-height: 200px; object-fit: cover;">
                        @else
                    ...
                    <div class="col-md-9">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <p class="mb-1"><strong>Penanggung Jawab:</strong> {{ $penerima_mbg->penanggung_jawab }}
                                </p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-1"><strong>Identitas:</strong> <span
                            ...
                            <div class="col-12">
                                ...
                                <p class="text-muted">
                                    ...
                                    @if ($penerima_mbg->village_name)
                                        Kel. {{ $penerima_mbg->village_name }},
                                    @endif
                                    @if ($penerima_mbg->district_name)
                                        Kec. {{ $penerima_mbg->district_name }},
                                    @endif
                            ...
                            @if ($penerima_mbg->latitude && $penerima_mbg->longitude)
                                <div class="col-md-6">
                                    <p class="mb-1"><strong>Koordinat:</strong> {{ $penerima_mbg->latitude }},
                                        {{ $penerima_mbg->longitude }}</p>
                                </div>
                            @endif
                            @if ($penerima_mbg->link_gmaps)
                                <div class="col-md-6">
                                    <a href="{{ $penerima_mbg->link_gmaps }}" target="_blank"
        ...
        @if ($penerima_mbg->status_approval === 'pending')
            <div class="d-flex gap-2">
                <form
                    action="{{ route('kepala-dapur.penerima-mbg.approve', ['dapur' => $dapur, 'penerima_mbg' => $penerima_mbg->id_penerima]) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <button type="submit" class="btn btn-success"><i class="bx bx-check me-1"></i> Setujui
                        Pengajuan</button>
                </form>
            ...
            <div class="modal fade" id="rejectModal" tabindex="-1">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <form id="rejectForm"
                            action="{{ route('kepala-dapur.penerima-mbg.reject', ['dapur' => $dapur, 'penerima_mbg' => $penerima_mbg->id_penerima]) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
